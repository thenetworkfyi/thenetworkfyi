...
4. **The sanitizer is a separate, narrowly-scoped step** (`memory/sanitize.py`): a
   deterministic PII strip (emails, phones) plus an optional higher-fidelity LLM pass with
   a *fixed* prompt and no tools. The component that sees raw cross-user data stays small
