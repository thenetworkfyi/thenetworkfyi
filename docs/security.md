...
4. **The sanitizer is a separate, narrowly-scoped step** (`memory/sanitize.py`): a
   mandatory Presidio pass redacts person names, email addresses, and phone numbers while
   keeping organizations and locations for search recall, plus an optional
   higher-fidelity LLM pass with a *fixed* prompt and no tools. Missing Presidio is a
   deployment error, not a silent downgrade. The component that sees raw cross-user data
   stays small and auditable; the main agent never self-censors.
