...
9. **Sender audit pseudonyms.** Audit correlation for a sender must use
   `security/sender_identifier.py`, which derives `snd_v1_...` tokens by HMAC-SHA256
   over a normalized sender address with the server-side `SENDER_IDENTIFIER_SECRET`.
   The digest is truncated before logging so audit entries can be correlated without
   storing raw email addresses or full keyed digests. Never log raw sender addresses,
   and never replace this with a bare `sha256(email)`: candidate-address dictionary
   lookup would make that reversible.
